...
- **MCP Server** — Model Context Protocol at `/mcp` for AI agents. Includes `assemble_context` for intelligent, intent-aware context assembly, plus composable tools for search, context, impact, and documents.
...
```
namespaces → principals → entities → edges
                                   → embeddings
                        → documents → embeddings
```

All tables carry `namespace_id` and are covered by RLS policies. Principals represent any caller identity — humans, AI agents, or services.
