...
func (r *StarRepositoryTestSuite) SetupSuite() {
	...
	r.assetRepository, err = postgres.NewAssetRepository(r.client, r.userRepository, postgres.DefaultMaxResultSize, "")
	if err != nil {
		r.T().Fatal(err)
	}
}
...
func (r *StarRepositoryTestSuite) TestCreate() {
	...
	r.Run("return invalid error if asset id is not uuid", func() {
		uid := uuid.NewString()
		id, err := r.repository.Create(r.ctx, r.ns, uid, "asset-id")
	...
	r.Run("return error user not found if user does not exist", func() {
		...
		id, err := r.repository.Create(r.ctx, r.ns, uid, createdAsset.ID)
...
func (r *StarRepositoryTestSuite) TestGetStargazers() {
	ownerEmail := "test-getstargazers@raystack.io"
	...
	r.Run("return list of users that star an asset if get users success", func() {
		err := setup(r.ctx, r.client)
		r.NoError(err)
		userID1, err := createUser(r.userRepository, r.ns, "user@raystack.io")
		r.NoError(err)

		createdAsset1, err := createAsset(r.assetRepository, r.ns, userID1, ownerEmail, "asset-urn-1", "table")
		r.NoError(err)

		id, err := r.repository.Create(r.ctx, r.ns, userID1, createdAsset1.ID)
		r.NoError(err)
		r.NotEmpty(id)

		userID2, err := createUser(r.userRepository, r.ns, "admin@raystack.io")
		r.NoError(err)

		id, err = r.repository.Create(r.ctx, r.ns, userID2, createdAsset1.ID)
		r.NoError(err)
		r.NotEmpty(id)
	...
	r.Run("return limited paginated list of users that star an asset if get users success", func() {
		...
		for i := 1; i < 20; i++ {
			userEmail := fmt.Sprintf("user%d@raystack.io", i)
			userID, err := createUser(r.userRepository, r.ns, userEmail)
			r.NoError(err)
			createdAsset, err := createAsset(r.assetRepository, r.ns, userID, ownerEmail, "asset-urn-1", "table")
			r.NoError(err)
...
func (r *StarRepositoryTestSuite) TestGetAllAssetsByUserID() {
	ownerEmail := "test-getallbyuserid@raystack.io"
	...
	r.Run("return list of starred assets if get by user id success", func() {
		...
		r.NoError(err)
		id, err = r.repository.Create(r.ctx, r.ns, userID1, createdAsset2.ID)
		r.NoError(err)
		r.NotEmpty(id)

		createdAsset3, err := createAsset(r.assetRepository, r.ns, userID1, ownerEmail, "asset-urn-3", "table")
		r.NoError(err)
		id, err = r.repository.Create(r.ctx, r.ns, userID1, createdAsset3.ID)
		r.NoError(err)
		r.NotEmpty(id)
...
func (r *StarRepositoryTestSuite) TestGetAssetByUserID() {
	ownerEmail := "test-getbyuserid@raystack.io"
...
func (r *StarRepositoryTestSuite) TestDelete() {
	ownerEmail := "test-delete@raystack.io"
